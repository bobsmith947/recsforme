...
public class UserBean implements Serializable {
	...
	public void addGroups(Result groups) {
		ListGroup group;
		for (SortedMap row : groups.getRows()) {
			group = new ListGroup(row.get("name").toString(),
						row.get("gid").toString(), row.get("type").toString());
			if ((Boolean) row.get("liked")) {
				likeData.getList().add(group);
			} else {
				dislikeData.getList().add(group);
			}
		}
	}

	public void removeGroups(Result groups) {
		ListGroup group;
		for (SortedMap row : groups.getRows()) {
			group = new ListGroup(row.get("name").toString(),
						row.get("gid").toString(), row.get("type").toString());
			likeData.getList().remove(group);
			dislikeData.getList().remove(group);
		}
	}
