	...
	<body>
		...
		<main>
							...
							<div class="modal-body">
								<c:if test="${u.tries < 6}" scope="session" var="notLocked">
									...
									<form data-bind="visible:!resetForm()" action="login.jsp" method="POST">
										...
										<c:if test="${!u.likeData.list.isEmpty() || !u.dislikeData.list.isEmpty()}">
									...
									<form data-bind="visible:resetForm,submit:requestReset" id="reset-form" method="POST">
										...
										<button data-bind="enable:pass()===check()&&pass()!==''" type="submit" class="btn btn-warning btn-lg btn-block mt-3" disabled>Reset Password</button>
									</form>
								</c:if>
								<c:if test="${!notLocked}">
			...
			<div class="text-right" id="recslist">
				...
				<c:if test="${r.recommendations == null || r.recommendations.list.isEmpty()}">
					<button id="recgen" type="button" class="btn btn-primary btn-block btn-lg my-4">Generate my recommendations</button>
				</c:if>
			...
			<div class="container">
				...
				<div class="row">
					<button class="btn btn-danger col-md my-3 mr-md-2 listreset" type="reset" data-list="like">Clear your likes list</button>
					<button class="btn btn-danger col-md my-3 listreset" type="reset" data-list="both">Clear both of your lists</button>
					<button class="btn btn-danger col-md my-3 ml-md-2 listreset" type="reset" data-list="dislike">Clear your dislikes list</button>
				</div>
				<div class="row">
					<button id="recsreset" class="btn btn-danger col" type="reset" disabled style="display:none">Clear your recommendations</button>
				</div>
			</div>
		</main>
	</body>
