<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
  <head>
    <meta name="keywords" content="recommendation engine, recommender system, open source, java, web app, web application">
    <meta name="description" content="recsforme (recs for me) is an open source Java web application for media recommendations.">
    <title>recsforme :: Home</title>
  </head>
  <body>
    <h3>Welcome to recsforme, a media recommendation web app.</h3>
    <p>recsforme is an open source project aimed at creating a fast and simple app for providing media recommendations. The source code is available on <a href="https://github.com/bobsmith947/recsforme">GitHub</a> under the <a href="https://github.com/bobsmith947/recsforme/blob/master/LICENSE">Apache License 2.0</a>. If you have a bug report, feature request, or anything else of that nature, be sure to give the <a href="https://github.com/bobsmith947/recsforme/blob/master/CONTRIBUTING.md">contributing guidelines</a> a read and submit an issue with any appropriate labels (and a clear title). If you're a Java/web developer, feel free to contribute code and make recsforme even better. More information on the project itself can be found <a href="http://info.recsfor.me">here</a>.</p>
    <h4>Alpha Testing</h4>
    <p>Thanks so much for deciding to become an alpha tester! First and foremost, I'd like to congratulate you on joining a team composed of some of the most excellent individuals on planet Earth. Secondly, I'd like to emphasize that you should open an issue or contact me about <i>anything</i> related to the project. Bug report, feature request, design improvement, etc., it's your duty to shape the future of the app, and if you don't say anything about what you want added/fixed/whatever, it may never be addressed. Once again, thank you so much, play around, and have fun!</p>
  </body>
</html>
