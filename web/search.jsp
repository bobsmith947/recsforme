...
<% if (q.getQuery().toLowerCase().equals("make me coffee")) {
			...
			<form id="adv-search" action="search.jsp">
				...
				<div class="input-group">
					...
					<select class="form-control form-control-lg" name="type">
						...
						<!--<option value="movie" <% if (t.equals("movie")) {
					out.print("selected");
				} %>>
						  Movies, Television Shows, and Video Games
						</option>-->
						<option value="artist" <% if (t.equals("artist")) {
					out.print("selected");
				} %>>
							Music Groups and People
						</option>
						<option value="album" <% if (t.equals("album")) {
					out.print("selected");
				}%>>
					...
					<c:if test="${del.different(oldQuery)}">
						<c:if test="${u.loggedIn}">
							<sql:update dataSource="jdbc/MediaRecom">
								INSERT INTO user_query (contents, type, user_id)
								VALUES (?, ?, ${u.id})
								<sql:param value="${q.query}" />
								<sql:param value="${q.type}" />
							</sql:update>
						</c:if>
						<c:if test="${!u.loggedIn}">
							<sql:update dataSource="jdbc/MediaRecom">
								INSERT INTO user_query (contents, type)
								VALUES (?, ?)
								<sql:param value="${q.query}" />
								<sql:param value="${q.type}" />
							</sql:update>
			...
			<div class="list-group text-center mb-2" id="results">
				<c:if test="${len > 0}">
					<c:forEach var="i" begin="0" end="${len-1}" step="1">
						<a class="list-group-item list-group-item-action" href="${q.context}${q.ids[i]}">
